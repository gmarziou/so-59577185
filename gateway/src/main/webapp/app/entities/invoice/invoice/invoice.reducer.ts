...
export const getSearchEntities: ICrudSearchAction<IInvoice> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_INVOICES,
  payload: axios.get<IInvoice>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
