...
import { IOrderItem, defaultValue } from 'app/shared/model/order/order-item.model';

export const ACTION_TYPES = {
  SEARCH_ORDERITEMS: 'orderItem/SEARCH_ORDERITEMS',
...
const apiUrl = 'services/order/api/order-items';
const apiSearchUrl = 'services/order/api/_search/order-items';

// Actions

export const getSearchEntities: ICrudSearchAction<IOrderItem> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_ORDERITEMS,
  payload: axios.get<IOrderItem>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
