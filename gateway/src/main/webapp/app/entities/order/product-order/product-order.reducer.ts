...
import { IProductOrder, defaultValue } from 'app/shared/model/order/product-order.model';

export const ACTION_TYPES = {
  SEARCH_PRODUCTORDERS: 'productOrder/SEARCH_PRODUCTORDERS',
...
export default (state: ProductOrderState = initialState, action): ProductOrderState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PRODUCTORDERS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PRODUCTORDERS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PRODUCTORDERS):
...
const apiUrl = 'services/order/api/product-orders';
const apiSearchUrl = 'services/order/api/_search/product-orders';

// Actions

export const getSearchEntities: ICrudSearchAction<IProductOrder> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PRODUCTORDERS,
  payload: axios.get<IProductOrder>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
